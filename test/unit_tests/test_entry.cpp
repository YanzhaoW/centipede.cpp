...
template <typename T>
using ValueError = centipede::ValueError<T>;
...
TEST(static_entrypoint, setters)
{
    ...
    auto is_equal =
        entry.get_locals() == centipede::EntryPoint<2, 3>::LocalDerivs{ ValueError{ 1.F }, ValueError{ 3.F } };
    EXPECT_TRUE(is_equal);
    is_equal = entry.get_globals() == centipede::EntryPoint<2, 3>::GlobalDerivs{ std::pair{ 3, ValueError{ 1.F } },
                                                                                 std::pair{ 10, ValueError{ 2.F } },
                                                                                 std::pair{ 11, ValueError{ 3.F } } };
...
TEST(dynamic_entrypoint, setters)
{
    ...
    auto is_equal = entry.get_locals() == centipede::EntryPoint<>::LocalDerivs{ ValueError{ 1.F }, ValueError{ 3.F } };
    EXPECT_TRUE(is_equal);
    is_equal = entry.get_globals() == centipede::EntryPoint<>::GlobalDerivs{ std::pair{ 3, ValueError{ 1.F } },
                                                                             std::pair{ 10, ValueError{ 2.F } },
                                                                             std::pair{ 11, ValueError{ 3.F } } };
    // NOLINTEND (cppcoreguidelines-avoid-magic-numbers)
    EXPECT_TRUE(is_equal);
...
TEST(dynamic_entrypoint, clear)
{
    ...
    EXPECT_EQ(entry.get_measurement(), ValueError{ 0.F });
}
...
TEST(dynamic_entrypoint, add_elements)
{
    ...
    entry.reserve_locals(3).add_local(1.).add_local(2.F).add_local(3.);
    auto local_test = std::vector{ ValueError{ 1.F }, ValueError{ 2.F }, ValueError{ 3.F } };
    EXPECT_EQ(entry.get_locals(), local_test);
    entry.reserve_globals(4).add_global(0, 2.).add_global(2, 2.2).add_global(4, 1.9).add_global(8, 2.8);
    auto global_test = std::vector{
        std::pair{ 0U, ValueError{ 2.F } },
        std::pair{ 2U, ValueError{ 2.2F } },
        std::pair{ 4U, ValueError{ 1.9F } },
        std::pair{ 8U, ValueError{ 2.8F } },
    };
